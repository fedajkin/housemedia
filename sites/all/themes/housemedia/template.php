 ...
 * Breadcrumb themeing
 */
function housemedia_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
 ...
 * Node preprocessing
 */
function housemedia_preprocess_node(&$vars) {
...
function housemedia_preprocess_page(&$vars) {
  ...
  $vars['header_link1'] = l(t('Link 1'), 'dashboard', array('attributes' => array(
    'id' => 'header-link1',
    'title' => t('Dashboard')
  )));
 ...
 */
function housemedia_preprocess_comment(&$vars) {
  global $user;
...
function housemedia_item_list($items = array(), $title = NULL, $type = 'ul', $attributes = NULL) {
 ...
 * Theme a box
 */
function housemedia_boxes_box($block) {
